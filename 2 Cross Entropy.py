...
def delta_An(X,Bn,h,k):
    return der_sigmoid(k)*(np.multiply(Bn,der_ReLU(h)))*(X.T)

def delta_an(X,Bn,h,k):
    return der_sigmoid(k)*(np.multiply(Bn,der_ReLU(h)))

def delta_Bn(X,Bn,h,k):
    return der_sigmoid(k)*h

def delta_bn(X,Bn,h,k):
    return der_sigmoid(k)

def delta_J_to_matrix(y0,y1,f):
    return (  (1/(1-y0))*f(t_input_0,Bn,h,k) - (1/y1)*f(t_input_1,Bn,h,k) )

def J_net1(y0,y1):
    return - math.log10(1-y0) - math.log10(y1)
...
Bn = np.array([[np.random.normal(0,(1/21))] for j in range(300)])    #300x1  
bn = 0
...
while(True):
    ...
    Jlist20.append(J_net1(y0,y1))  # holds 20 costs to take average
...
print("Iterations to train network 1 = %d" %iterations)

plt.xlabel('Every 20nth iteration')
plt.ylabel('Cost in log scale')
plt.title('Convergence of Network 1')
plt.plot(Jlist)
plt.yscale('log')
plt.show()

# end of training network 1
    
An_opt_net1 = Ann  
an_opt_net1 = ann
Bn_opt_net1 = Bnn
bn_opt_net1 = bnn
θ_opt_net1 = [An_opt_net1,an_opt_net1,Bn_opt_net1,bn_opt_net1] #θο of network 1
...
while(i<973):      
    ...
    z = np.dot(An_opt_net1,x_input_0) + an_opt_net1
    h = ReLU(z)
    k = ( np.dot(Bn_opt_net1.T,h) + bn_opt_net1 )[0][0]       
    y0 = sigmoid(k)  # output of network 1 for input x coming from HO 

    if(y0>=0.5):
        s01 = s01 + 1

    x_input_1 = (np.array([eights_test[i]])).T #forwarding input coming from H1
    z = np.dot(An_opt_net1,x_input_1) + an_opt_net1     
    h = ReLU(z)
    k = ( np.dot(Bn_opt_net1.T,h) + bn_opt_net1 )[0][0]        
    y1 = sigmoid(k)  # output of network 1 for input x coming from H1 
...
# end of testing network 1

p_error_net1_H0 = s01 / 974   
p_error_net1_H1 = s10 / 974

print("Ποσοστό σφάλματος για την υπόθεση H0 του δικτύου 1 (cross entropy) = %f" % p_error_net1_H0)
print("Ποσοστό σφάλματος για την υπόθεση H1 του δικτύου 1 (cross entropy) = %f" % p_error_net1_H1)
print("Αθροιστικό ποσοστό σφάλματος = %f" % ((p_error_net1_H0 + p_error_net1_H1)/2))
