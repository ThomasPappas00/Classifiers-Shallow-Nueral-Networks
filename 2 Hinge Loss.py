...
def delta_J_to_matrix(y0,y1,f):
    return (  der_Φ(y0)*f(t_input_0,Bn,h,k) + der_Ψ(y1)*f(t_input_1,Bn,h,k) )

def J_net3(y0,y1):
    return max(1+y0,0) + max(1-y1,0)
...
while(True):
    ...
    Jlist20.append(J_net3(y0,y1))  # holds 20 costs to take average 
...
print("Finished training network 3")
print("Iterations to train network 3 = %d" %iterations)

plt.xlabel('Every 20nth iteration')
plt.ylabel('Cost in log scale')
plt.title('Convergence of Network 3')
plt.plot(Jlist)
plt.yscale('log')
plt.show()

# end of training network 3
    
An_opt_net3 = Ann  
an_opt_net3 = ann
Bn_opt_net3 = Bnn
bn_opt_net3 = bnn
θ_opt_net3 = [An_opt_net3,an_opt_net3,Bn_opt_net3,bn_opt_net3] #θο of network 3
...
while(i<973):      
    ...
    z = np.dot(An_opt_net3,x_input_0) + an_opt_net3
    h = ReLU(z)
    k = ( np.dot(Bn_opt_net3.T,h) + bn_opt_net3 )[0][0]       
    ...
    z = np.dot(An_opt_net3,x_input_1) + an_opt_net3     
    h = ReLU(z)
    k = ( np.dot(Bn_opt_net3.T,h) + bn_opt_net3 )[0][0]        
...
# end of testing network 3

p_error_net3_H0 = s01 / 974   
p_error_net3_H1 = s10 / 974

print("Ποσοστό σφάλματος για την υπόθεση H0 του δικτύου 1 (hinge loss) = %f" % p_error_net3_H0)
print("Ποσοστό σφάλματος για την υπόθεση H1 του δικτύου 1 (hinge loss) = %f" % p_error_net3_H1)
print("Αθροιστικό ποσοστό σφάλματος = %f" % ((p_error_net3_H0 + p_error_net3_H1)/2))
